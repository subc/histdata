...
class Command(BaseCommand):
    ...
    def write(self, cls_ma, write_history, candles_m5, candles_d1):
        ...
        for index in xrange(index_max):
            ...
            ma = self.calc_ma(index, cls_ma, candles_m5, candles_d1)
    ...
    def calc_ma(self, index, cls_ma, candles_m5, candles_d1):
        # ma計算
        ma = cls_ma(open_bid=candles_m5[index].open_bid,
                    start_at=candles_m5[index].start_at)
        # ma追加
        for key in KEYS:
            count = PAIR.get(key)
